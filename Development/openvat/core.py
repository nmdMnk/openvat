...
def get_scene_compositor_tree(
    scene: bpy.types.Scene, ensure: bool = True, name: str | None = None
):
    """
...
def get_compositor_output_socket(
    tree: bpy.types.NodeTree, ensure_interface: bool = True
):
    ...
    if tree.bl_idname == "CompositorNodeTree" and hasattr(
        bpy.context.scene, "compositing_node_group"
    ):
        out_node = None
        for n in tree.nodes:
...
def setup_alpha_over_node(
    ...
    for inp in alpha_over_node.inputs:
        if (
            getattr(inp, "identifier", "") == "straight_alpha"
            or inp.name == "Straight Alpha"
        ):
            straight_input = inp
            break
    ...
    rl_out = (
        render_layers_node.outputs.get("Image", None) or render_layers_node.outputs[0]
    )
...
def setup_vat_tracker(
    ...
    if use_custom:
        ...
        if (
            bpy.data.scenes[original_scene.name].vat_settings.vat_normal_encoding
            == "PACKED"
        ):
...
def setup_proxy_scene(
    obj, num_frames, width, height, num_wraps, temp_obj, pack_normals, framestart
):
    ...
    image_result = bpy.data.images[
        obj.name.replace("_ovbake", "") + "_vat" + image_extension
    ]
...
def setup_vat_scene(
    ...
    setup_compositing(
        vat_scene, output_dir, vat_scene.name, proxy_obj, image_format, fmt
    )
    ...
    output_path = os.path.join(
        output_dir, f"{output_name}", f"{output_name}{image_format}"
    )
    ...
    if not pack_normals:
        if bpy.data.scenes[original_scene_name].vat_settings.encode_type == "DEFAULT":
            if (
                bpy.data.scenes[original_scene_name].vat_settings.vat_normal_encoding
                != "NONE"
            ):
                ...
                tree = get_scene_compositor_tree(
                    vat_scene, ensure=True, name=f"{vat_scene.name}_vnrm"
                )
...
def setup_compositing(
    vat_scene, output_dir, scene_name, proxy_obj, image_format, raw_format
):
    tree = get_scene_compositor_tree(
        vat_scene, ensure=True, name=f"{vat_scene.name}_Compositor"
    )
    if tree is None:
...
def render_vat_scene(vat_scene, num_frames, output_dir, image_format, raw_format):
    ...
    nrmoutput_path = os.path.join(
        output_dir,
        f"{output_name}",
        vat_scene.name.replace("_vat", "_vnrm") + image_format,
    )
...
def render_vat_nrml(vat_scene, num_frames, output_dir, image_format, raw_format):
    ...
    output_path = os.path.join(
        output_dir, f"{output_name}", f"{rendername}{image_format}"
    )
    img_settings = vat_scene.render.image_settings
...
def export_vat_model(file_format="FBX", include_materials=False, include_tangents=True):
    ...
    export_directory = bpy.path.abspath(
        bpy.context.scene.vat_settings.vat_output_directory
    )
    ...
    if file_format.upper() == "FBX":
        ...
        raise ValueError(
            f"Unsupported export format: {file_format}. Use 'FBX', 'GLB', or 'GLTF'."
        )
