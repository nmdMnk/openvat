...
class OBJECT_OT_CalculateVATResolution(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        export_directory = bpy.path.abspath(
            context.scene.vat_settings.vat_output_directory
        )
        selected_temp = None
        ...
        if settings.encode_type == "CUSTOM":
            ...
            if missing:
                self.report(
                    {"ERROR"},
                    f"Missing attributes: {', '.join(missing)}. Please rescan.",
                )
                return {"CANCELLED"}
        ...
        if settings.encode_type != "CUSTOM":
            if (
                settings.encode_target == "COLLECTION_COMBINE"
            ):  # Collection target only valid when collection_mode is true
                collection_mode = True
                collection_target = settings.vat_collection.name

        core.create_geo_nodes_bake(
            use_collection=collection_mode, collection_name=collection_target
        )
        obj = context.view_layer.objects.active
        obj_name = obj.name

        # VAT normals are per-vertex data, so hard normal discontinuities need
        # split vertices to survive encoding (especially fractured cut faces).
        ...
        output_filepath = os.path.join(
            object_directory, f"{output_rename}-remap_info.json"
        )

        # Saturation sampling output
        remap_output_filepath = os.path.join(
            object_directory, f"{output_rename}-remap_info.json"
        )
        ...
        if settings.encode_type == "DEFAULT":
            ...
            utils.make_remap_data(
                obj_name,
                attribute_name,
                frame_start,
                frame_end,
                output_filepath,
                remap_output_filepath,
                anim_list,
                "",
            )

            min_x, min_y, min_z, max_x, max_y, max_z = utils.read_remap_info(
                remap_output_filepath, attribute_name
            )
            ...
            utils.make_custom_data(
                obj_name,
                [attr_r, attr_g, attr_b],
                frame_start,
                frame_end,
                output_filepath,
                remap_output_filepath,
            ...
            min_r, min_g, min_b, max_r, max_g, max_b = utils.read_custom_info(
                remap_output_filepath, attrs
            )
        ...
        if settings.encode_type == "CUSTOM":
            ...
            if settings.use_single_row:
                ...
                width, height, num_wraps = utils.calculate_optimal_vat_resolution(
            ...
            if settings.use_single_row:
                ...
                width, height, num_wraps = utils.calculate_packed_vat_resolution(
                    num_vertices, num_frames
                )
        else:
            if settings.use_single_row:
        ...
        print(
            "Thank you for using OPENVAT - Your favorite Vertex Animation Encoder - Developed by Luke Stilson 2024 - Visit www.lukestilson.com for more information"
        )
...
classes = [
    OBJECT_OT_CalculateVATResolution,
    OBJECT_OT_OpenOutputDirectory,
    OBJECT_OT_ScanFloatPointAttributes,
]
