...
class VATSettings(bpy.types.PropertyGroup):
    ...
    vat_transform: bpy.props.EnumProperty(
        ...
        items=[
            (
                "ORIGINAL",
                "Object",
                "Resulting mesh does not take object transforms into account, apply transforms on target object for best results",
            ),
            (
                "RELATIVE",
                "World",
                "Target object's world transforms are baked into VAT data, origin of VAT mesh is (0,0,0). Animated or current world transforms become deformation data",
            ),
        ],
        default="ORIGINAL",
    )
    ...
    proxy_method: bpy.props.EnumProperty(
        ...
        items=[
            (
                "START_FRAME",
                "Start Frame",
                "The target's deformation as it appears in the first encoded frame becomes the Proxy",
            ),
            (
                "CURRENT_FRAME",
                "Current Frame",
                "The target's deformation as it appears in the scene's current frame becomes the Proxy",
            ),
            (
                "SELECTED_OBJECT",
                "Selected Object",
                "A single selected object in scene may be chosen to be used as the proxy, valid only when Target and Proxy share exact vert/face count",
            ),
        ],
        default="START_FRAME",
    )
    ...
    vat_cleanup_enabled: bpy.props.BoolProperty(
        name="Perform Cleanup", description="Removes all generated data", default=True
    )
    ...
    encode_type: bpy.props.EnumProperty(
        ...
        items=[
            ...
            (
                "CUSTOM",
                "Custom Attributes (float[3])",
                "Choose custom attributes to encode to RGB",
            ),
        ],
        default="DEFAULT",
    )
    ...
    mesh_format: bpy.props.EnumProperty(
        ...
        items=[
            ...
            (
                "GLTF",
                "glTF Separate",
                "Export vat-compatible as .gltf + .bin + textures",
            ),
        ],
        default="FBX",
    )
    ...
    unreal_axis_convention: bpy.props.BoolProperty(
        name="Use Unreal Axis Convention",
        description="Export FBX with Y Forward / Z Up axes to match Unreal Engine / Send to Unreal convention",
        default=False,
    )

    export_unreal_box_collision: bpy.props.BoolProperty(
        name="Export Unreal Box Collision",
        description="Export a generated UBX box collision mesh with FBX model exports for Unreal Engine",
        default=False,
    )
    ...
    encode_target: bpy.props.EnumProperty(
        ...
        items=[
            (
                "ACTIVE_OBJECT",
                "Active Object",
                "Encode the active object's Animation to VAT",
            ),
            (
                "COLLECTION_COMBINE",
                "Collection (combined)",
                "Encode all the objects in a specified collection, combined into a single object",
            ),
        ],
    ...
    image_format: bpy.props.EnumProperty(
        ...
        items=[
            ...
            (
                "EXR16",
                "EXR16",
                "Recommended | OpenEXR Float (half) in ZIP Codec with 16 bit color-depth | RGB16(half)",
            ),
            (
                "EXR32",
                "EXR32",
                "OpenEXR Float (half) Full Precision in ZIP Codec with 32 bit color-channels | RGB16(full)",
            ),
        ],
        default="EXR16",
    )
    ...
    vat_normal_encoding: bpy.props.EnumProperty(
        ...
        items=[
            (
                "NONE",
                "None",
                "Only encode position data, do not export vertex normals data",
            ),
            ("PACKED", "Packed", "Pack vertex normals into the same VAT as position"),
            (
                "SEPARATE",
                "Separate Map",
                "Use a separate texture output for vertex normals",
            ),
        ],
        default="PACKED",
    )
